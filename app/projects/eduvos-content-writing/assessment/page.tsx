    ...
    <div className="min-h-screen bg-surface">
      ...
      <section className="bg-surface">
        <div className="max-w-3xl mx-auto px-6 py-20 md:py-28">
          <Link
            ...
            className="inline-flex items-center gap-2 text-sm text-ink/82 hover:text-ember transition-colors duration-200 mb-16 group"
          >
          ...
          <div className="flex flex-wrap gap-1.5 mb-6">
            {["EdTech", "Content Design", "Formative Assessment"].map((tag) => (
              <span key={tag} className="text-xs text-ink/82">{tag}</span>
            ))}
          </div>
          ...
          <p className="text-lg text-ink/82 max-w-xl leading-relaxed">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 mt-14">
            {[
              ...
              <div key={item.label} className={`p-6 min-w-0 overflow-hidden border-line ${
                ...
                <p className="text-xs text-ink/82 font-medium uppercase tracking-widest mb-2">{item.label}</p>
      ...
      <section className="bg-accent-wash">
        <div className="max-w-3xl mx-auto px-6 py-10">
          <p className="text-xs text-ink/82 font-medium uppercase tracking-widest mb-4">Learning Objective</p>
          ...
          <div className="space-y-6">
            {[
              ...
              <div key={i} className="flex items-start gap-6">
                <span className="text-xs text-ink/75 font-bold mt-0.5">{String(i + 1).padStart(2, "0")}</span>
                <p className="text-sm text-ink/90 leading-relaxed">{req}</p>
              </div>
            ))}
          </div>
        </section>
            ...
            {exercises.map((ex) => (
              <div key={ex.label} className="bg-surface rounded-lg p-8 md:p-10">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="text-xs text-ink/82 font-medium uppercase tracking-widest">{ex.label}</span>
                </div>
                ...
                <div className="space-y-3 mb-8">
                  {ex.instructions.map((ins, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <p className="text-sm text-ink/88 leading-relaxed">{ins}</p>
                    </div>
                  ))}
                </div>
                <div className="grid md:grid-cols-2 gap-6 pt-6 border-t border-line">
                  <div>
                    <p className="text-xs text-ink/82 font-medium uppercase tracking-widest mb-2">Deliverable</p>
                    <p className="text-sm text-ink/90 leading-relaxed">{ex.deliverable}</p>
                  </div>
                  <div>
                    <p className="text-xs text-ink/82 font-medium uppercase tracking-widest mb-2">Tools</p>
                    <p className="text-sm text-ink/90 leading-relaxed">{ex.tools}</p>
                  </div>
                </div>
              </div>
            ))}
