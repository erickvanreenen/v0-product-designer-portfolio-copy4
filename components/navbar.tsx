...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-paper/90 backdrop-blur-md border-b border-line">
      <nav className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
            ...
            {navItems.map((item) => (
              <li key={item.href}>
                ...
                >
                  ...
                  <span
                    className={cn(
                      "absolute -bottom-[18px] left-0 right-0 h-[2px] origin-left transition-transform duration-200",
                      pathname === item.href
                        ? "bg-ember scale-x-100"
                        : "bg-ink/25 scale-x-0 group-hover/nav:scale-x-100"
                    )}
                  />
                </Link>
              </li>
          ...
          <Link
            ...
            className={`transition-colors duration-200 ${pathname === "/about" ? "text-ember hover:text-ember/70 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
          >
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="mobile-menu-open md:hidden bg-paper border-b border-line">
